...
const DataService = {
    ...
    async fetchCommits(repo) {
        try {
            let allCommits = [];
            let page = 1;
            // Fetch up to 5 pages (500 commits) which usually covers 6 months for most thesis projects
            while (page <= 5) {
                const resp = await fetch(`https://api.github.com/repos/${repo}/commits?per_page=100&page=${page}`);
                if (!resp.ok) break;
...
const Sidebar = ({ activeTab, setActiveTab, isCollapsed, setCollapsed, mobileOpen, closeMobileMenu, title, profile, team, onOpenProfile, onOpenMember }) => {
    ...
    return (
        ...
        >
            ...
            <div className={`px-4 py-3 border-t border-white/10 transition-all duration-300 ${(!mobileOpen && isCollapsed) ? 'hidden' : 'block'}`}>
                <div className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-3 pl-2">Supervision Team</div>
                <div className="space-y-2">
                    {team && team.length > 0 ? (
                        team.map(member => (
                            <div key={member.id} onClick={() => onOpenMember && onOpenMember(member)} className="flex items-center gap-3 p-2 rounded-lg hover:bg-white/40 transition-colors cursor-pointer">
                                <div className={`w-8 h-8 rounded-full flex items-center justify-center shrink-0 border 
                                    ${member.role === 'Supervisor' ? 'bg-indigo-50 text-indigo-500 border-indigo-100' : 'bg-teal-50 text-teal-500 border-teal-100'}`}>
                                    <i className={`fas ${member.role === 'Supervisor' ? 'fa-chalkboard-teacher' : 'fa-user-check'} text-xs`}></i>
                                </div>
                        ...
                        <div className="p-2 text-xs text-gray-400 text-center italic">No team members added</div>
                    )}
                </div>
            </div>

            <div 
                className={`p-4 border-t border-white/20 flex items-center gap-3 transition-all duration-300 cursor-pointer hover:bg-white/50 ${(isCollapsed && !mobileOpen) ? 'justify-center mx-1' : ''}`}
                onClick={onOpenProfile}
            >
                <div className="w-10 h-10 rounded-full bg-gray-200 overflow-hidden shrink-0 border-2 border-white shadow-sm">
                    <img src={userProfile.image} alt={userProfile.name} className="w-full h-full object-cover" onError={(e) => e.target.src = 'https://ui-avatars.com/api/?name=Faizan&background=random'} />
                </div>
                <div className={`overflow-hidden transition-all duration-300 ${(isCollapsed && !mobileOpen) ? 'w-0 opacity-0' : 'w-auto opacity-100'}`}>
...
const StatCard = ({ title, value, footer, icon, color = "blue", onClick }) => {
    const gradients = {
    ...
    return (
        <div 
            onClick={onClick}
            className={`glass p-6 rounded-2xl hover-lift relative overflow-hidden group ${onClick ? 'cursor-pointer hover:ring-2 hover:ring-white/50' : ''}`}
        >
...
const NotesFeed = ({ notes, onOpenProfile }) => {
    return (
            ...
            <div className="flex-1 overflow-y-auto space-y-4 pr-2 custom-scrollbar max-h-[500px]">
                {notes.map((note, i) => (
                    <div key={i} className="flex gap-4 animate-slide-up bg-white/40 p-4 rounded-xl border border-white/50" style={{ animationDelay: `${i * 0.1}s` }}>
                        <div className="shrink-0 cursor-pointer" onClick={() => note.author.toLowerCase() === 'faizan' && onOpenProfile && onOpenProfile()}>
...
const Dashboard = ({ config, commits, onOpenProfile, onOpenMember, onNavigate }) => {
    return (
        <div className="p-4 md:p-8 max-w-7xl mx-auto space-y-6 animate-fade-in pb-20">
            <header className="mb-10 text-center md:text-left border-b border-gray-200/50 pb-6">
                <h1 className="text-4xl md:text-5xl font-extrabold text-gray-900 tracking-tight mb-4 leading-tight">
                    {config.config?.dashboardTitle || "Geospatial Tagging of Volunteered Place Descriptions"}
                </h1>
                <p className="text-xl text-gray-600 font-light">
                    Bayesian Elastic Net & Extended FP-Growth Approach
                </p>
                
                <div className="mt-4 grid grid-cols-1 md:grid-cols-2 gap-4 max-w-lg mx-auto md:mx-0">
                    {config.team && config.team.length > 0 ? (
                        config.team.map(member => (
                            <div 
                                key={member.id} 
                                onClick={() => onOpenMember(member)}
                                className="flex items-center gap-3 bg-white/50 p-2 rounded-lg border border-white/60 cursor-pointer hover:bg-white/80 hover:shadow-sm transition-all duration-200 group"
                            >
                                <div className={`w-10 h-10 rounded-full flex items-center justify-center shrink-0 transition-transform group-hover:scale-110
                                    ${member.role === 'Supervisor' ? 'bg-indigo-100 text-indigo-600' : 'bg-teal-100 text-teal-600'}`}>
                                    <i className={`fas ${member.role === 'Supervisor' ? 'fa-chalkboard-teacher' : 'fa-user-check'}`}></i>
                                </div>
                                <div>
                                    <p className="text-xs text-gray-500 font-bold uppercase">{member.role}</p>
                                    <p className="text-sm font-bold text-gray-800 group-hover:text-blue-600 transition-colors">{member.name}</p>
                                </div>
                            </div>
                        ))
                    ) : (
                         <div className="col-span-2 text-center text-gray-400 italic text-sm">No supervisors listed</div>
                    )}
                </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <StatCard
                    ...
                    onClick={() => window.open(`https://github.com/${config.config?.githubRepo}/commits`, '_blank')}
                />
                <StatCard
                    title="Reports"
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div>
                    <NotesFeed notes={config.notes || []} onOpenProfile={onOpenProfile} />
                </div>
            </div>
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
            ...
            <div className="bg-white rounded-3xl shadow-2xl w-full max-w-md relative z-10 overflow-hidden animate-slide-up">
                ...
                <div className="h-32 bg-gradient-to-r from-blue-600 to-purple-600 relative">
                    <button onClick={onClose} className="absolute top-4 right-4 text-white/80 hover:text-white transition-colors bg-black/20 hover:bg-black/40 rounded-full p-2">
                        <i className="fas fa-times text-xl block w-6 h-6 text-center leading-6"></i>
                    </button>
                </div>
                    ...
                    <div className="grid grid-cols-4 gap-4 mb-8">
                         {profile.email && (
                            <a href={`mailto:${profile.email}`} className="flex flex-col items-center gap-1 group">
                                ...
                                <span className="text-[10px] uppercase font-bold text-gray-400 group-hover:text-blue-600 transition-colors">Email</span>
                            </a>
                        )}
                        {profile.github && (
                            <a href={profile.github} target="_blank" className="flex flex-col items-center gap-1 group">
                                <div className="w-10 h-10 rounded-full bg-gray-100 text-gray-700 flex items-center justify-center group-hover:bg-gray-800 group-hover:text-white transition-all shadow-sm">
                                    <i className="fab fa-github"></i>
                                </div>
                                <span className="text-[10px] uppercase font-bold text-gray-400 group-hover:text-gray-800 transition-colors">GitHub</span>
                            </a>
                        )}
                        {profile.linkedin && (
                            <a href={profile.linkedin} target="_blank" className="flex flex-col items-center gap-1 group">
                                <div className="w-10 h-10 rounded-full bg-blue-50 text-blue-700 flex items-center justify-center group-hover:bg-blue-700 group-hover:text-white transition-all shadow-sm">
                                    <i className="fab fa-linkedin-in"></i>
                                </div>
                                <span className="text-[10px] uppercase font-bold text-gray-400 group-hover:text-blue-700 transition-colors">LinkedIn</span>
                            </a>
                        )}
                        {profile.twitter && (
                            <a href={profile.twitter} target="_blank" className="flex flex-col items-center gap-1 group">
                                <div className="w-10 h-10 rounded-full bg-sky-50 text-sky-500 flex items-center justify-center group-hover:bg-sky-500 group-hover:text-white transition-all shadow-sm">
                                    <i className="fab fa-twitter"></i>
                                </div>
                                <span className="text-[10px] uppercase font-bold text-gray-400 group-hover:text-sky-500 transition-colors">Twitter</span>
                            </a>
                        )}
                    </div>
...
const OsmPage = () => {
    return (
        <div className="p-4 md:p-8 max-w-7xl mx-auto space-y-6 animate-fade-in pb-20 text-center">
            <h1 className="text-4xl md:text-5xl font-extrabold text-gray-900 tracking-tight mb-4 leading-tight">
                OpenStreetMap Tools & Analytics
            </h1>
            <p className="text-xl text-gray-600 font-light mb-8">
                Explore, visualize, and analyze OpenStreetMap data with our upcoming features.
            </p>
            <div className="bg-gradient-to-r from-blue-50 to-indigo-50 p-10 rounded-3xl shadow-xl border border-blue-100 max-w-2xl mx-auto">
                <p className="text-5xl font-extrabold text-blue-600 mb-6 animate-pulse">
                    Coming Soon!
                </p>
                <p className="text-lg text-gray-700 leading-relaxed">
                    We're actively developing advanced tools for geospatial data exploration, pattern analysis, and enhanced visualization using OpenStreetMap data. This section will feature interactive maps, custom data querying, and deep-dive analytics to help users understand urban landscapes and their descriptions. Stay tuned for exciting updates!
                </p>
                <div className="mt-8 text-blue-500 text-3xl space-x-6">
            ...
            <p className="mt-12 text-gray-500 text-sm">
                Expected features include: interactive map interface, data filtering, pattern visualization, and user-driven analysis.
            </p>
        </div>
    );
};
...
const App = () => {
    ...
    const renderContent = () => {
        switch (activeTab) {
            case 'dashboard': return <Dashboard config={config} commits={commits} onOpenProfile={() => setProfileOpen(true)} onOpenMember={setSelectedMember} onNavigate={setActiveTab} />;
            case 'osm_page': return <OsmPage />; // New case
            case 'reports': return <ReportsViewer />;
            case 'data': return <DataExplorer config={config.config} />;
            default: return <Dashboard config={config} commits={commits} onOpenProfile={() => setProfileOpen(true)} onOpenMember={setSelectedMember} onNavigate={setActiveTab} />;
        }
    };
